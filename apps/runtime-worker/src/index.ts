...
function makeMcpAdapter(): McpAdapter {
  return {
    async execute(actionType, run, degraded) {
      try {
...
function collectOutput(
  ...
  return formatTemplateOutput(template.type, merged);
}

export class RuntimeWorker {
  private readonly adapters: AdapterRegistry;

  constructor(private readonly traceLog = new InMemoryTraceLog()) {
    this.adapters = {
  ...
  private async executeTemplate(
    ...
    const template = getTemplateByType(run.templateType);
    validateTemplateInput(run.templateType, run.input);
    ...
    for (const action of template.supportedActions) {
      ...
      if (action.adapter === "mcp") {
        result = await this.adapters.mcp.execute(action.actionType, run, degraded);
    ...
    const completed = {
      ...
      outputPayload: collectOutput(template, actionResults),
      currentStep: null
    };
